...
export default function App() {
  ...
  async function handleBrowserEvent(event: BrowserEvent, node: any) {
    switch (event.type) {
      ...
      case 'receivedFileChunk': {
        ...
        updateFileInChat(file.storageKey, { bytesComplete: file.bytesComplete, completed: file.completed })
      ...
      case 'receivedFileCompleted': {
        ...
        upsertReceivedFile(file)
        updateFileInChat(file.storageKey, { bytesComplete: file.sizeBytes, completed: true })
      ...
      case 'error': addToast('error', event.message); break
    }
  }
  ...
  async function handleSendFile(file: File) {
    ...
    try {
      ...
      updateFileInChat(storageKey, { bytesComplete: bytes.byteLength, completed: true })
    } catch (err) {
      addToast('error', `File send failed: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
      ...
      <main class="app-main">
        ...
        <aside class={`app-sidebar ${state.sidebarOpen ? 'is-open' : ''}`}>
          <div class="app-sidebar__top">
            <CodeInput code={state.code} onCodeChange={setCode} onSubmit={handleCodeSubmit} onGenerate={handleGenerateCode} />
            <PeerBanner />
          </div>
